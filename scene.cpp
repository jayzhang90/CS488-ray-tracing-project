...
bool SceneNode::is_moving() const
{
	const int c= get_move();
	std::cout<<c<<"sdass\n";
  if(c ==3) return true;
  return false;
...
/*
  if(is_moving() ==true){
  	...
  	origin =Point3D(t* origin[0]+(1-t)*(origin[0]+1),t* origin[1]+(1-t)*(1+origin[1]),t* origin[2]+(1-t)*(1+origin[2]));
  	//Point3D c1= t*origin+(1-t)*origin;
  	std::cout<<"moveing\n";
  }
  */
