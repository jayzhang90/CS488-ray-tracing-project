...
Hit Material::apply_bump_map(Hit h) {
  ...
  */
  ...
  auto bumpmap = [this](double u, double v) -> double { 
    double di = u * (double)(m_bump_map.width()-1);
    double dj = v * (double)(m_bump_map.height()-1);

    int i = di;
    int j = dj;

    // Wrap around if the coordinates are out of bounds
    int i1 = ((i + 1) >= m_bump_map.width()) ? 0 : i + 1;
    int j1 = ((j + 1) >= m_bump_map.height()) ? 0 : j + 1;
    ...
    return (1-up)*(1-vp)*m_bump_map(i, j, 0) +
        (1-up)*(vp)*m_bump_map(i, j1, 0) +
        (up)*(1-vp)*m_bump_map(i1, j, 0) +
        (up)*(vp)*m_bump_map(i1, j1, 0);
  };
  double u=h.tex_coords[0];
double v=h.tex_coords[1];
  ...
  Vector3D D = (0.5 / N.length()) * (fu * (N.cross(h.pv)) - fv * (N.cross(h.pu)));
